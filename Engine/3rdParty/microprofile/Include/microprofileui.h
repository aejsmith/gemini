...
#if GEMINI_PROFILER // Adjust reference times.
static float g_MicroProfileReferenceTimePresets[] = {5.f, 10.f, 16.66f, 33.33f, 66.66f, 100.f, 250.f, 500.f, 1000.f};
#else
...
#if GEMINI_PROFILER // Missing column border
uint32_t MicroProfileDrawBarCallCount(int32_t nX, int32_t nY, const char* pName, uint32_t nTotalHeight)
{
	MicroProfileDrawLineVertical(nX-5, 0, nTotalHeight+nY, UI.nOpacityBackground|g_nMicroProfileBackColors[0]|g_nMicroProfileBackColors[1]);
#else
...
#if GEMINI_PROFILER // Small tweak to hide a redundant border
        nX += 5;
#else
		nX += 10;
#endif
...
#if GEMINI_PROFILER // Missing column border
        nX += MicroProfileDrawBarCallCount(nX, nY, "Count", nTotalHeight) + 1; 
#else
